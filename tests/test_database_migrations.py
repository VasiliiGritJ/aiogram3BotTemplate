...
class DatabaseMigrationTests(unittest.TestCase):
    ...
    def test_migration_ten_adds_durable_format_blocks_without_touching_legacy_rows(self) -> None:
        ...
        self.assertEqual((10,), run_migrations(session.engine, target_version=10))
        self.assertEqual((), run_migrations(session.engine, target_version=10))
        inspector = inspect(session.engine)
        self.assertTrue({
    ...
    def test_migration_eleven_adds_user_program_metadata_with_legacy_defaults(self) -> None:
        session = self.make_session("stage-seven-user-programs.db")
        run_migrations(session.engine, target_version=10)
        with session.engine.begin() as connection:
            connection.exec_driver_sql(
                "INSERT INTO users (id, tg_id, fullname, username, inviter_id) "
                "VALUES (1, 9200, 'Programs User', 'programs', 0)"
            )
            connection.exec_driver_sql(
                "INSERT INTO workout_templates (id, code, name, goal, experience_level, "
                "workouts_per_week, duration_bucket, equipment) VALUES "
                "(1, 'legacy-program', 'Legacy', 'muscle_gain', 'beginner', 1, 'short', 'gym')"
            )
            connection.exec_driver_sql(
                "INSERT INTO user_workout_plans (id, user_id, template_id, profile_signature) "
                "VALUES (1, 1, 1, 'legacy')"
            )
            connection.exec_driver_sql(
                "INSERT INTO workout_sessions "
        ...
        with session.engine.connect() as connection:
            ...
            workout = connection.exec_driver_sql(
                "SELECT plan_source, adaptation_mode FROM workout_sessions"
            ).one()
